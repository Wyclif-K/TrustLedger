...
const navItems = [
  ...
  { to: '/pending-requests',  icon: ClipboardList,   label: 'Pending requests' },
...
export default function Sidebar({ mobileOpen = false, onClose }) {
  ...
  return (
    <aside
      className={clsx(
        'fixed top-0 left-0 flex flex-col print:hidden',
        'h-dvh max-h-dvh w-[min(85vw,var(--sidebar-w))] sm:w-[var(--sidebar-w)] max-w-[var(--sidebar-w)]',
        'overflow-hidden bg-white border-r border-navy-100',
      ...
      id="app-sidebar"
    >
      <div className="shrink-0 px-4 sm:px-5 py-4 sm:py-5 bg-gradient-to-br from-navy-900 to-navy-950 text-white border-b border-navy-800 flex items-start gap-2">
      ...
      {/* Mobile: scroll nav + footer together. Desktop: nav scrolls, footer pinned. */}
      <div
        className={clsx(
          'flex-1 min-h-0 flex flex-col overflow-hidden',
          'max-lg:overflow-y-auto max-lg:overflow-x-hidden max-lg:overscroll-y-contain',
          'max-lg:touch-pan-y max-lg:[-webkit-overflow-scrolling:touch]',
          'max-lg:pb-[max(1rem,env(safe-area-inset-bottom))]'
        )}
      >
        <nav
          className={clsx(
            'px-2 sm:px-3 py-4 space-y-0.5 bg-surface-50/50',
            'lg:flex-1 lg:min-h-0 lg:overflow-y-auto lg:overflow-x-hidden',
            'lg:overscroll-y-contain lg:touch-pan-y lg:[-webkit-overflow-scrolling:touch]'
          )}
        >
        ...
        <div className="shrink-0 px-3 pb-4 max-lg:pb-2 space-y-1 border-t border-navy-100 pt-3 bg-white">
